...
def perform():
    ...
    from pymri.dataset import DatasetManager2
    # dataset settings
    ds = DatasetManager2(
        ...
        contrast=[var_class_00.get().split(' '), var_class_00.get().split(' ')],
        normalize = True,
        nnadl = True,
        )
    # load data
    ...
    # print(var_bold.get())
    # print(var_attr.get())
    # print(var_attr_lit.get())
    # print(var_mask_brain.get())
    # print(var_output.get())

    # print(var_class_00.get().split(' '))
    # print(var_class_01.get().split(' '))
...
norm_load_chk = tk.Checkbutton(
           ...
           command=cv_choose, variable=var_norm_load,
           font=standard_font
           )
norm_load_chk.grid(
...
feat_sel_ex = tk.IntVar()
n_features = tk.IntVar()

n_features_frame = tk.Frame(
    feature, borderwidth = font_size/6
    )
n_features_frame.grid(
    padx=20, pady=20, ipadx=10, ipady=10
    )

n_features_label = tk.Label(
    n_features_frame, text="Extract/select n features:", font=standard_font
    )
n_features_label.grid(row=0, column=0, sticky='W')

n_features_entry = tk.Entry(n_features_frame, width=4, font=standard_font)
...
ROI_entry = tk.Entry(
    ROI_frame, width=40, font=standard_font, textvariable=var_bold
    )
